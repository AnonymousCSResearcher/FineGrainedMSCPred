...
def load_indexes():
    ...
    indexes = {}
    index_files = ['msc_keyword_idx.json', 'keyword_msc_idx.json',
                    'msc_refs_idx.json', 'refs_msc_idx.json']
    for index_file in index_files:
        ...
        try:
            ...
            # import generate_indexes_notebook
            # print('generate keyword index')
            # generate_indexes_notebook.generate_idx('keyword')
            # print('generate reference index')
            # generate_indexes_notebook.generate_idx('refs')
...
def predict_text_mscs(table,indexes):
    ...
    # open index
    #sorted_msc_keyword_idx = indexes['msc_keyword_idx.json']
    sorted_keyword_msc_idx = indexes['keyword_msc_idx.json']
        ...
        for n in n_gram_lengths:
            try:
                ...
                for nngram in nngrams:
                    ...
                    entity = entity[:-1]
                    try:
                        if sorted_keyword_msc_idx[entity] is not None and entity not in sstopwords:
                            # print(entity)
                            # mscs_predicted[idx].extend(list(sorted_keyword_msc_idx[entity])[0:1])
                            for cls in sorted_keyword_msc_idx[entity].items():
                                try:
                                    # SELECTION HERE
                                    mscs_predicted_stat[
...
def predict_keyword_mscs(table,indexes):
    ...
    for idx in range(tot_rows):
        ...
        for keyword in keywords:
            try:
                for cls in sorted_keyword_msc_idx[keyword].items():
                    try:
                        # SELECTION HERE
                        mscs_predicted_stat[
                            cls[0]] += 1  # cls[1]#1 # weightedcontribution or binarycontribution
...
def predict_reference_mscs(table,indexes):
    ...
    # open index
    #sorted_msc_refs_idx = indexes['msc_refs_idx.json']
    sorted_refs_msc_idx = indexes['refs_msc_idx.json']
    ...
    for idx in range(tot_rows):
        ...
        for reference in references:
            try:
                for cls in sorted_refs_msc_idx[reference].items():
                    try:
                        # SELECTION HERE
                        mscs_predicted_stat[
                            cls[0]] += 1  # cls[1]#1 # weightedcontribution or binarycontribution
...
def load_data():
    # load data
    # msc(keyword) index
    with open('data/keyword_msc_idx.json', 'r') as f:
        keyword_msc_index = json.load(f)
    # raw data
    raw_data = None  # pd.read_csv('data/')
...
def get_dcg_table(raw_data, mrmscs_dict, keyword_msc_index):
    ...
    eval_table.to_csv('data/keywords_vs_refs_mrmscs.csv')
...
def compare_mr_keyword_refs_dcgs(raw_data):
    ...
    # eval eval table
    print('Score eval')
    eval_table = pd.read_csv('data/keywords_vs_refs_mrmscs.csv')
